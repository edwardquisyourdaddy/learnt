...
def conv2d_forward(x, w, b, pad, stride):
    ...
    num_of_filters, filter_height, filter_width = w.shape[0], w.shape[1], w.shape[2]
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width = ((width - filter_width + 2 * pad) // stride) + 1
    out = np.zeros([x.shape[0], new_height, new_width, b.shape[0]])

    for i in range(batch):
        #padding first
        data_new = np.zeros((height + 2*pad, width + 2*pad, channels))
        for j in range(channels):
            data_new[:,:,j] = np.pad(x[i,:,:,j], pad, 'constant')
        ...
        for k in range(num_of_filters):
            ...
            for m in range(new_height):
                for n in range(new_width):
                    process_data = data_new[m*stride:m*stride+filter_height, n*stride:n*stride+filter_width,:]
                    out[i,m,n,k] = np.sum(process_data*neuron_weight) + bias
