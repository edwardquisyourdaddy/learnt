...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k,v in params.items():
            self.cache[k] = gamma * cache[k] + (1 - gamma) * np.square(grads[k])
            params[k] -= np.multiply(np.divide(learning_rate, np.sqrt(self.cache[k] + eps)), grads[k]) 
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            self.momentums[k] = beta1 * momentums[k] + (1 - beta1) * grads[k]
            self.velocitys[k] = beta2 * velocitys[k] + (1 - beta2) * np.square(grads[k])
            momentums_corrected = np.divide(self.momentums[k], 1 - beta1**t)
            velocitys_corrected = np.divide(self.velocitys[k], 1 - beta2**t)
            params[k] -= learning_rate * momentums_corrected / (np.sqrt(velocitys_corrected) + eps)
        #raise NotImplementedError